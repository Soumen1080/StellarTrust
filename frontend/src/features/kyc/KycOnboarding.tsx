...
export function KycOnboarding() {
  ...
  return <div className="grid items-start gap-lg lg:grid-cols-[minmax(0,1fr)_340px]">
    <form onSubmit={submit} className="panel-light overflow-hidden">
      <div className="border-b border-hairline-light p-md sm:p-lg md:p-xl"><div className="flex flex-wrap items-start justify-between gap-md"><div><p className="eyebrow">Verification application</p><h2 className="mt-xs text-2xl font-bold">Applicant details</h2><p className="mt-sm max-w-2xl text-sm leading-6 text-muted">Use the sandbox controls to exercise approval and human-review paths. Sensitive values are not written to application logs.</p></div><span className="rounded-pill border border-info/30 bg-info/10 px-sm py-xs text-xs font-semibold text-info">Sandbox mode</span></div></div>
      ...
      <div className="p-md sm:p-lg md:p-xl">
        ...
        <div className="mt-xl flex flex-col-reverse gap-md border-t border-hairline-light pt-lg sm:flex-row sm:items-center sm:justify-between sm:gap-sm"><p className="flex items-center gap-xs text-xs text-muted"><Icon name="lock" className="h-4 w-4" />Encrypted in transit · idempotent submission</p><button type="submit" disabled={pending} className="btn-primary w-full justify-center sm:w-auto sm:min-w-[190px]">{pending ? "Running checks…" : "Submit verification"}<Icon name={pending ? "clock" : "arrow-right"} className="h-4 w-4" /></button></div>
      </div>
    </form>

    <aside className="space-y-lg lg:sticky lg:top-24">
      <section className="panel-light p-lg"><div className="flex items-center justify-between"><p className="text-sm font-semibold">Profile status</p>{profile ? <StatusPill status={profile.user.kycStatus} /> : <span className="text-xs text-muted">Loading…</span>}</div>{profile?.wallets[0] ? <div className="mt-md rounded-lg bg-surface-strong-light p-sm"><p className="data-label">Connected wallet</p><p className="mt-xs break-all font-mono text-xs" title={profile.wallets[0].stellarPublicKey}>{profile.wallets[0].stellarPublicKey}</p></div> : null}{profile?.business ? <p className="mt-md text-sm font-medium">{profile.business.legalName}</p> : null}</section>
