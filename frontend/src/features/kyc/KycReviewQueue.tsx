...
export function KycReviewQueue() {
  ...
  return <div>
    ...
    {visible.length === 0 && !error ? <div className="panel-dark px-lg py-xxl text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-surface-elevated-dark text-status-verified"><Icon name="check" /></span><h2 className="mt-md font-semibold text-on-dark">{reviews.length ? "No matching reviews" : "No reviews queued"}</h2><p className="mx-auto mt-xs max-w-lg text-sm text-muted">{reviews.length ? "Adjust the current filter or search term." : "Borderline, conflicting, low-confidence, and provider-failure cases will appear here."}</p></div> : <div className="space-y-md">{visible.map((review) => {
      ...
      <div className="mt-lg grid gap-md border-t border-hairline-dark pt-md sm:grid-cols-3"><div><p className="text-[10px] uppercase tracking-wider text-muted">Provider checks</p><p className={`mt-xs text-sm font-medium ${failedChecks ? "text-status-review" : "text-status-verified"}`}>{failedChecks ? `${failedChecks} require attention` : "All passed"}</p></div><div><p className="text-[10px] uppercase tracking-wider text-muted">Signals considered</p><p className="mt-xs text-sm text-body">{review.advisory.signals.length} explainability signal{review.advisory.signals.length === 1 ? "" : "s"}</p></div><div className="sm:text-right"><button type="button" aria-expanded={expanded} aria-controls={`review-details-${review.id}`} onClick={() => setExpandedId(expanded ? null : review.id)} className="text-sm font-semibold text-primary">{expanded ? "Hide case details" : "Review case details"}<Icon name="chevron-down" className={`ml-xs inline h-4 w-4 transition ${expanded ? "rotate-180" : ""}`}/></button></div></div></div>
