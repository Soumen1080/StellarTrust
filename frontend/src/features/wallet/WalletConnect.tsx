...
export function WalletConnect() {
  ...
  if (session) {
    ...
    return <div className="flex max-w-2xl flex-col gap-sm">
      <div className="flex flex-col gap-sm sm:flex-row sm:items-center"><div className="flex min-h-11 items-center gap-sm rounded-md border border-hairline-dark bg-surface-card-dark px-md"><span className="grid h-7 w-7 place-items-center rounded-full bg-status-verified/10 text-status-verified"><Icon name="wallet" className="h-4 w-4" /></span><div><p className="text-[10px] uppercase tracking-wider text-muted">Wallet connected</p><p className="font-mono text-xs text-body" title={key}>{key.slice(0, 7)}…{key.slice(-6)}</p></div></div><Link href={isVerified ? "/dashboard" : "/kyc"} className="btn-primary">{loading ? "Checking account…" : isVerified ? "Open dashboard" : "Continue verification"} <Icon name="arrow-right" className="h-4 w-4" /></Link><button type="button" onClick={() => void disconnect()} disabled={pending} className="min-h-10 px-sm text-sm font-medium text-muted-strong transition hover:text-on-dark disabled:opacity-50">{pending ? "Disconnecting…" : "Disconnect"}</button></div>
