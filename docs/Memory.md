...
- **Current phase:** Phase 6 — Hardening. Application-level hardening is
  ...
  dispute auto-execution). Phases 1–5 application code is complete. The escrow
  on-chain path — the largest outstanding gap from Phase 2 — is now implemented.
  What remains is operational/external: a funded testnet identity + deployed
  contracts, a real KMS signer, Redis, live anchors, an independent security
  audit, and MSB licensing.

### Verified state (2026-08-18 — re-measured, not assumed)
...
| `ai` — pytest | ⚠️ CI-only (native wheels blocked on this machine) |
| Migrations `0001`–`0008` executed | ❌ never run locally (no psql/Docker) |

> Two long-standing notes in this file were **wrong** and are corrected here:
> `cargo test` **does** run on this Windows machine (all 27 contract tests
> pass), and the backend suite has **no** failing tests. The three "pre-existing
> KYC failures" were config-validation failures caused by a placeholder
> `AUTH_DEMO_WALLET` in the local `.env`; blank env vars now mean "unset" and
> `vitest.config.ts` isolates demo/chain vars.

- **Repo state:** All portions present. Postgres repositories exist and are used
  when `DATABASE_URL` is set for: identity, auth, audit, payments, disputes,
  rwa. Still in-memory in every environment: kyc, reputation, settlement, and
  all idempotency stores. Chain adapters have real Soroban implementations for
  escrow and RWA; anchor and liquidity are still sandbox/deterministic only.
...
- **What was built (on-chain escrow — 2026-08-18):**
  - **Root problem:** the escrow contract was written and tested but
    *unreachable*. `SorobanRpcEscrowGateway.submitTransition` threw
    unconditionally, `ESCROW_WASM_HASH` had zero readers, no code resolved a
    user id to a Stellar address, and nothing validated a strkey anywhere.
  - **Contract** (`contracts/escrow`): added an on-chain `order_ref` so
    reconciliation can prove a contract id belongs to its order; allowed the
    **arbiter** to raise a dispute (previously `dispute()` was callable by
    nobody in practice, which made the arbiter's release path unreachable — the
    contract accepts `release` only from `Disputed` or `delivery_confirmed`);
    added `Error::Unauthorized`. 9 tests pass.
  - **Signing split.** `initialize`/`confirm_delivery`/`dispute` need the acting
    party's `require_auth()`, so they run **prepare → wallet sign → submit**
    with that party's account as the transaction source (one envelope signature
    covers it — asserted via `needsNonInvokerSigningBy()`). `release`/`refund`
    are arbiter-signed server-side. `GET /api/payments/capabilities` publishes
    the split so the frontend never hard-codes it.
  - **New boundaries:** `modules/stellar/address.ts` (StrKey validation —
    previously *nothing* validated a Stellar address), `modules/stellar/asset.ts`
    (exact integer minor-units ↔ token amounts, refuses to truncate),
    `modules/escrow/escrow.addresses.ts` (user id → SEP-10-proven wallet).
  - **Correctness:** every submission reads the contract back and checks state +
    order binding before the ledger posts; `getEscrowSnapshot` distinguishes
    `isErr()` (contract says "not initialized") from a thrown error (RPC down),
    so an outage cannot be mistaken for missing custody.
  - **Divergence fixed:** the deterministic adapter used to let `arbiter: true`
    ...
    `InvalidState`. It now mirrors the contract, and `settleDisputedOrder`
    escalates to `Disputed` first. Regression-tested.
  - **Schema:** `EscrowState.Pending` (custody deployed, buyer has not signed)
    and `PaymentTransition.Dispute`, via migration `0008`. Persisting the
    contract id at prepare-time means an abandoned signature costs one idle
    contract instead of one per retry.
  - **Config now enforces itself:** `superRefine` fails boot when
    `ESCROW_GATEWAY=soroban-rpc` lacks a WASM hash or token binding (the
    comments always claimed this; nothing checked it).
  - **Frontend:** `useEscrowOrders` drives prepare → sign → submit with honest
    per-phase status, polls every 12s while orders are in flight, pauses on a
    hidden tab, and re-reads after an error because the step may have landed
    on-chain anyway.
  - **Also fixed:** the deploy script ran `contract deploy` (emits a contract
    id) when both gateways need the WASM *hash* — it now runs `contract upload`
    for both contracts and prints the env lines.
  - **Test isolation:** 9 of 11 test files were failing *before* this work from
    a placeholder `AUTH_DEMO_WALLET` in the local `.env`. Blank env vars now
    mean "unset" and `vitest.config.ts` clears demo/chain vars.
- **What was built (Phase 6 — Hardening, application layer):**
  - **Observability.** `lib/metrics.ts`: dependency-free Prometheus registry
    (Counter/Gauge/Histogram) exposing `http_requests_total`,
    `http_request_duration_seconds`, `reconciliation_unresolved_mismatches`,
    `reconciliation_runs_total`, `alerts_total`. `middleware/metrics.ts` records
    per-route request count + latency with bounded cardinality (matched route
    pattern, never ids/PII). `/metrics` (Prometheus text), `/health/live`
    (liveness), and `/health/ready` (DB ping + reconciliation drift → 503 when
    degraded) endpoints. Per-request id already provides trace correlation.
  - **Alerting.** `lib/alerts.ts`: `AlertSink` interface + `LoggingAlertSink`
    (structured log + metric) + `RecordingAlertSink` (tests). Both reconciliation
    jobs now emit a critical alert on unresolved drift, set the unresolved gauge
    + run counter, and track `lastUnresolved()` for the readiness probe.
  - **Reputation store** (`modules/reputation`, the Phase 4 deferral): bounded
    0..1 advisory score from completed orders and resolved disputes
    (Laplace-smoothed toward a neutral 0.5 prior). `ReputationService`
    (getScore/getReputation/recordOrderCompleted/recordDisputeOutcome, all
    audited) + `InMemoryReputationRepository` + read-only `/api/reputation`
    routes (self, or compliance for any user). Advisory only — never gates money.
    Wired into the dispute advisory (real buyer/seller priors) and updated on
    every resolved dispute (winner/loser) and every completed order.
  - **Dispute auto-execution** (the Phase 4 "release-path state-machine work"):
    `PaymentService.settleDisputedOrder(orderId, outcome, actor)` — an
    arbiter/system-gated path that releases or refunds a *locked* escrow on the
    authority of a resolved dispute (adds an `arbiter` flag to the escrow
    gateway so release no longer requires prior buyer confirmation; uses a
    balanced escrow→custody ledger override; a release also fires any linked RWA
    payout). `DisputeService.resolve` now auto-executes the resolved outcome
    through a `DisputeSettlementGateway` port — best-effort, audited
    (`dispute.settlement_executed` / `dispute.settlement_failed`), and
    retryable, so the dispute record stays the authority and a failure never
    rolls back the resolution.
  - **Design boundary upheld:** AI + reputation remain advisory; money moves
    only through the audited, reconciled arbiter payments path; every action is
...
- **Verification status:** see the table in §1 — it is re-measured, not carried
  forward. Two standing caveats:
  - ⚠️ `ai`: `pytest` is not installable on this machine (native wheels blocked
    by OS policy); CI is authoritative. The engines remain weighted-rule
    placeholders, not models.
...
| D45 | 2026-08-18 | Transitions the escrow contract gates with a counterparty's `require_auth()` (`lock`, `confirm`, `dispute`) use a **prepare → wallet sign → submit** round trip, with that party's account as the transaction source | The server cannot produce those signatures. Source-account credentials satisfy `require_auth()` with a single envelope signature, so no separate auth-entry signing is needed |
| D46 | 2026-08-18 | The signing model is published at runtime via `GET /api/payments/capabilities`, never hard-coded in the client | Switching `ESCROW_GATEWAY=deterministic → soroban-rpc` must not require a frontend change |
| D47 | 2026-08-18 | The server deploys the custody contract **before** the buyer signs the lock, and persists the contract id immediately (`EscrowState.Pending`) | An abandoned signature then costs one idle contract rather than leaking a fresh one on every retry |
| D48 | 2026-08-18 | For any chain-backed step, the chain transaction must settle **and be read back** (state + `order_ref`) before the ledger posts | A rejected or misdirected transaction must never leave the books claiming custody that does not exist |
| D49 | 2026-08-18 | The escrow contract accepts the **arbiter** as a disputing party | `release` accepts only `Disputed` or `delivery_confirmed`; without this the arbiter's settlement path was unreachable on-chain, and compliance would be blocked on a signature from the party it is ruling against |
| D50 | 2026-08-18 | A deterministic adapter must reject exactly what the real contract rejects; divergences are regression-tested | The old adapter allowed an arbiter release of an unconfirmed escrow that the contract rejects with `InvalidState` — a green suite that proved nothing about deployment |
| D51 | 2026-08-18 | A dispute changes custody state and writes audit, but posts **no** ledger entries | No money moves. Forcing a balanced posting would have meant inventing accounts or zero-amount entries the schema forbids |
| D52 | 2026-08-18 | User id → Stellar address resolves **only** from the SEP-10-proven wallet, never from client input; all addresses are StrKey-validated at the boundary | A client-supplied address would let a caller redirect custody or a payout to an account they chose |
...
| Persistence (partial) | identity/auth/audit/payments/disputes/rwa are Postgres-backed when `DATABASE_URL` is set. **kyc, reputation, settlement, and every idempotency store are still in-memory in all environments.** | Write the remaining repositories; idempotency needs Redis to work across instances at all |
| Idempotency across instances | `InMemoryIdempotencyStore` cannot dedupe between processes, so Golden Rule #4 is not actually enforced in a multi-instance deploy | Redis-backed store, or pin to a single instance until then |
| Migrations never executed | `0001`–`0008` have been written and reviewed but never run — no psql/Docker locally | Run them against a real Postgres in CI or a provisioned DB before trusting any persistence claim |
...
- Verified present: Node v24.18.0, npm 11.16.0, Python 3.14.6, git 2.55.0,
  and a working Rust/cargo toolchain.
- ✅ **`cargo test` works here** — all 27 contract tests run and pass locally.
  (This file previously claimed Windows cargo was blocked inside Soroban macro
  dependencies. That is no longer true; do not skip contract tests.)
- ⚠️ Python native (Rust-backed) wheel builds remain blocked by OS policy, so
  `pytest` for `ai/` is CI-only.
- ⚠️ No Docker/psql locally → migrations and DB triggers have **never been
  executed**, only reviewed. Treat every persistence claim as unverified until
  they run.
- ⚠️ No Stellar CLI identity funded here → nothing on-chain has been executed
